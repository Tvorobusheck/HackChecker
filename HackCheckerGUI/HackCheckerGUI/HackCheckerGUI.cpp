...
{
	PAINTSTRUCT ps;
	static HWND hBtnChoice; // дескриптор кнопки
	static HWND hBtnCheck; // дескриптор кнопки
    ...
    case WM_COMMAND:
        {
			if (lParam == (LPARAM)hBtnChoice) {
				...
				if (sFile.checkFile()){
					MessageBoxW(0, file_path.c_str(), L"Опасный файл!", 0);
				}
				else {
					MessageBoxW(0, file_path.c_str(), L"Безопасный файл", 0);
				}
			}
